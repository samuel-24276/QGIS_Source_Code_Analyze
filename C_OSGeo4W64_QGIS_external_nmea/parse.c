...
/**
解析NMEA标准数据里的时间，解析成功返回0，失败返回-1。参数buff是被解析的时间数据，buff_sz是解析数据的长度，
根据不同长度把时间数据解析成不同格式（如hhmmss、hhmmss.s、hhmmss.ss、hhmmss.sss等)。
*/
...
/**
 * \brief Fill nmeaINFO structure by GGA packet data.通过GGA数据包数据填充nmeaINFO结构
