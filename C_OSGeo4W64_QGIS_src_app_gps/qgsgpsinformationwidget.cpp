...
const int MAXACQUISITIONINTERVAL = 3000; // max gps information acquisition suspension interval (in seconds)最大gps信息获取暂停时间间隔（以秒为单位）
const int MAXDISTANCETHRESHOLD = 200; // max gps distance threshold (in meters)最大gps距离阈值（以米为单位）
...
{
  Q_ASSERT( mMapCanvas ); // precondition前提
  setupUi( this );
...
QgsCoordinateTransform用于在两个地图坐标系之间进行转换的类。此类可以将地图坐标转换为其他坐标参考系统。
它通常与地图图层相关联，并且可以在图层的坐标系和地图画布的坐标系之间进行转换，尽管可以在更一般的意义上使用它来转换坐标。
当用于在图层和地图画布之间进行转换时，所有对源坐标系和目标坐标系的引用都分别指图层和地图画布。 所有操作都是从该层的角度进行的。 例如，正向变换
将坐标从图层的坐标系转换到地图画布。
*/
